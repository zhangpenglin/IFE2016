(function () {
    ...
    function Node(data) {
        ...
        this.element.addEventListener('click', function (e) {
            clearColor()
            selectedNode = this
            addClass(this.element, "selected")//设置选择样式
            if (this.children.length > 0) {//如果有子节点 设置展开
                if (this.expand == false) {
                    this.expand = true
                    removeClass(this.element, "parent_right")
                    addClass(this.element, "parent_up")
                } else {
                    this.expand = false
                    removeClass(this.element, "parent_up")
                    addClass(this.element, "parent_right")
                }
    ...
    Tree.prototype.add = function (data, toData, traversal) {
        ...
        if (parent) {
            ...
            addClass(parent.element, "parent_up")
        }
    ...
    t.add("Book", "Bed", t.traverseDF)
    t.add("School", "Bed", t.traverseDF)
    ...
    function animate(nodeArr, searchMode) {
        ...
        function animation(nodeArr, searchMode) {
            ...
            addClass(current.element, "current")
            setTimeout(function () {
                if (searchMode && (current.data == $("#search").value)) {
                    hasSearchresult = true
                    addClass(current.element, "search");

                    /**
                     * 设置父节点展开状态
                     */
                    (function setParentExpand(current) {
                        if (current.parent) {
                            if (!current.parent.expand) {
                                current.parent.expand = true
                                removeClass(current.parent.element,"parent_right")

                                addClass(current.parent.element,"parent_up")
                            }
                            setParentExpand(current.parent)
                        }
                    })(current)
                } else {
                    removeClass(current.element, "current")
                    removeClass(current.element, "search")
                }
                animation(nodeArr, searchMode)
            }, 500)
        }
    ...
    function clearColor() {
        [].slice.call(document.querySelectorAll(".node")).forEach(function (ele) {
            ...
            removeClass(ele, "search")
            removeClass(ele, "selected")
        })
    }

    function addClass(ele, className) {
        var has = ele.className.split(" ").indexOf(className) > -1
        if (!has) {
